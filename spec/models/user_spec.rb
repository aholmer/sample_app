...
describe User do
	...
	describe "return value of authenticate method" do
		before { @user.save }
		let(:found_user) { User.find_by(email: @user.mail) }

		describe "with valid password" do
			it { should eq found_user.authenticate("invalid") }
		end
	end
	...
	describe "when email format is is invalid" do
		it "should be valid" do
			addresses = %w[user@foo,com user_at_foo.org 
						example.user@foo. foo@bar_baz.com 
						foo@bar+baz.com @au.com a@foo@bar.com]	
